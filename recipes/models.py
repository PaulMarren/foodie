...
class Category(models.Model):
    ...
    """
    name = models.CharField(max_length=50, unique=True) # Enforce unique category names
    slug = models.SlugField(max_length=50, unique=True, blank=True) # URL-friendly version of name
    description = models.TextField(blank=True) # Optional description of the category

    class Meta:
        verbose_name_plural = "Categories" # Proper plural name for admin interface
...
class Recipe(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True) # Auto-set date on creation
    ...
    featured_image = CloudinaryField('image', default='placeholder') # Cloudinary-hosted main image
...
class Ingredient(models.Model):
    ...
    quantity = models.CharField(max_length=50, blank=True) # Optional measurement
